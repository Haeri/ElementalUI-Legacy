...
const std::map<std::string, int> Solver::operator_level =
{
	{"(", 0},
	{"[", 0},
	{"{", 0},
...
float Solver::eval(const std::string& expr)
{
	...
	if (!tokenize(expr, tokens)) return err;
	if (!is_balanced(tokens)) return err;
	if (!to_postfix(tokens, postfix)) return err;
	if (!eval_postfix(postfix, result)) return err;
		...
		else
		{
			if (stack != "")
			{
				if (std::count(stack.begin(), stack.end(), '.') > 1)
				{
					...
					std::cerr << "Syntax Error: Malformed number at position " << number_index << std::endl;
					return false;
				}
...
bool Solver::is_balanced(const std::vector<Token>& tokens)
{
		...
		if (it->type == Type::BRACE)
		{
			...
			else if (operator_level.at(it->value) == 4)
			{
				...
				else
				{
					...
					std::cerr << "Syntax Error: Missmatched or unnecessary closing brace at position " << it->position << std::endl;
					return false;
				}
			}
		}
	...
	if (!stack.empty())
	{
		...
		std::cerr << "Syntax Error: Missing corresponding closing braces for '" << stack.back().value << "' at position " << stack.back().position << std::endl;
		return false;
	}
	...
	for (auto it = tokens.begin(); it != tokens.end(); ++it)
	{
		...
		else if (it->type == Type::OPERATOR)
		{
			...
			else
			{
				...
				if (VERBOS)
				{
					std::cout << "\t" << left << " " << it->value << " " << right << " = " << stack.back() << std::endl;
				}
			}
		}
	}
