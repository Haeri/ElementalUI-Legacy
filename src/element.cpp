...
void elem::element::add_child(element* child)
{
	_children.push_back(child);
	child->set_parent(this);
}

void elem::element::remove_child(int index)
{
	element* elem = _children[index];
	remove_child(elem);
}

void elem::element::remove_child(element* child)
{
	_children.erase(std::remove(_children.begin(), _children.end(), child), _children.end());
	child->set_parent(nullptr);
}

float elem::element::layout(elemd::vec2 position, float width)
{
	_position = position;
...
void elem::element::paint(elemd::Context* ctx)
{
...
void elem::element::set_parent(element* parent)
{
	_parent = parent;
}
